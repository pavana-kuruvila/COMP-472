...
# dataset path
datasetPath = "/Users/ektapatel/Desktop/COMP-472/DataSet2"
...
data_transforms = transforms.Compose([
    ...
    transforms.ColorJitter(brightness=(0.5, 1.5), contrast=(
        0.5, 1.5), saturation=(0.5, 1.5), hue=(-0.1, 0.1)),
    transforms.GaussianBlur(kernel_size=3, sigma=2),
    transforms.RandomGrayscale(p=0.1),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[
                         0.229, 0.224, 0.225])  # Standard from ImageNet
])
...
trainSet, testSet, validSet = random_split(dataset, [0.7, 0.15, 0.15])

trainLoader = DataLoader(trainSet, shuffle=True, batch_size=32)
testLoader = DataLoader(testSet, shuffle=True, batch_size=32)
validLoader = DataLoader(validSet, shuffle=True, batch_size=32)
...
class CNN(nn.Module):
    ...
    def forward(self, x):
     ...
     print("After conv_layer:", x.shape)
     # flatten to 2d
     x = x.view(x.size(0), -1)
...
# calculate validation loss
    ...
    validation_loss = 0
    with torch.no_grad():
     for images,labels in validLoader: #Iterate through validation dataset
        outputs = model(images)
        loss= criterion(outputs,labels) #compute loss
        validation_loss+=loss.item()*images.size(0) #accumulate validation loss
     validation_loss/= len(validLoader.dataset) #average loss
     print(f'Epoch [{epoch+1}/{num_epochs}], Validation Loss: {validation_loss}')

#earlystopping
    #If current validation loss is lower than minimum seen
